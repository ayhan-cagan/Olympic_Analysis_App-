...
st.sidebar.image("https://i2.wp.com/tdf.gov.tr/wp-content/uploads/2021/07/olimpiyat2.jpg?fit=800%2C400&ssl=1")
user_menu = st.sidebar.radio("Select an Option",
("Medal Tally","Overall Analysis","Country-Wise Analysis","Atlhete-Wise Analysis"))
...
if user_menu == "Overall Analysis":
    ...
    sport_list = df["Sport"].unique().tolist() # select box için kod 
    sport_list.sort()
...
if user_menu == "Atlhete-Wise Analysis":
    ...
    for sport in famous_sport:
        temp_df = atlhete_df[atlhete_df["Sport"]==sport]
    ...
    st.title("Distribution of Age with Sports(Gold Metalist)")
    st.plotly_chart(fig)
    ...
    ax = sns.scatterplot(x="Weight", y="Height", data=temp_df,hue=temp_df["Medal"],style=temp_df["Sex"],s=100) 
    st.pyplot(fig)
